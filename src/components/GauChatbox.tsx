...
export default function GauChatbox() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const backupKey = (customApiKey || "").trim();
      const actualKey = envKey || backupKey;

      if (actualKey) {
        headers["X-Gemini-Key"] = actualKey;
      }
          ...
          <div className="bg-gradient-to-r from-amber-400 to-orange-400 p-4 flex justify-between items-center text-white border-b-4 border-amber-500">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-amber-50 font-bold flex items-center gap-1">
                  ...
                  {customApiKey ? "🔑 Có khóa dự phòng riêng" : "Nhà khoa học vui tính 🌿"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <button
                ...
                className={`w-8 h-8 rounded-full ${showSettings ? 'bg-white/40' : 'bg-white/20'} hover:bg-white/40 flex items-center justify-center transition-colors cursor-pointer mr-1`}
          ...
          {showSettings ? (
            <div className="flex-1 p-5 overflow-y-auto bg-[#FFFDF2] flex flex-col gap-4 text-zinc-800">
              ...
              <div className="text-[11px] leading-relaxed text-zinc-650 font-bold bg-amber-50/50 border border-amber-200/40 p-3 rounded-2xl">
                Hệ thống ưu tiên đọc khóa từ <strong className="text-emerald-700 font-black">import.meta.env.VITE_GEMINI_API_KEY</strong>. 
                Nếu không tìm thấy, cô dán chìa khóa dự phòng tự điền của cô tại đây làm dự phòng nhé:
                <ul className="list-disc pl-4 mt-2 space-y-1">
                  <li>Khóa lưu cục bộ tại trình duyệt của cô (<strong className="text-amber-900 font-black">Local Storage</strong>).</li>
                  <li>Truyền đi bảo mật bằng tiêu đề <strong className="text-emerald-750">X-Gemini-Key</strong>.</li>
                </ul>
              </div>

              <div className="flex flex-col gap-1 mt-2">
                <label className="text-[10px] font-black uppercase text-zinc-500 block">
                  Nhập Khóa Gemini Dự Phòng Của Cô:
                </label>
                ...
                <p className="text-[10px] font-bold text-zinc-450 italic mt-1 leading-snug">
                  * Nhấn Lưu để lưu khóa vào trình duyệt riêng tư của cô nhé.
                </p>
              </div>
